        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-400 dark:text-gray-600 space-y-4">
              ...
              <button onClick={onClose} className="mt-4 text-primary-600 hover:text-primary-700 font-semibold">
                Start Shopping
              </button>
            </div>
          ) : (
            items.map(item => (
              <div key={item.id} className="flex gap-4 border border-gray-100 dark:border-gray-800 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow group">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-semibold text-gray-900 dark:text-white line-clamp-2 text-sm">{item.title}</h3>
                    <p className="text-primary-600 font-black mt-1">${item.price.toFixed(2)}</p>
                  </div>
...
function ShoppingBagIcon(props: React.SVGProps<SVGSVGElement>) {
  return (
    <svg fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" {...props}>
