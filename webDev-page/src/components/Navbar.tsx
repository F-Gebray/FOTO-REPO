...
const NavItem = ({ item }: { item: typeof navItems[0] }) => {
  ...
  return (
    ...
    >
      <a href={item.href} className="text-sm text-gray-300 hover:text-white transition-colors flex items-center gap-1">
        {item.name}
        ...
        {isOpen && (
          ...
          >
            {item.dropdown.map((sub, i) => {
              const isRouterLink = sub.href.startsWith('/') && !sub.href.includes('#');
              const linkClasses = "block px-4 py-2.5 text-sm font-medium text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition-colors";
            ...
            <div className="px-4 pt-2 pb-6 space-y-4 shadow-xl max-h-[80vh] overflow-y-auto">
              {navItems.map((item, idx) => (
                <div key={idx} className="pb-2">
                  <span className="block px-3 py-2 text-base font-bold text-white uppercase tracking-wider">{item.name}</span>
                  <div className="pl-4 border-l-2 border-white/5 ml-3 space-y-1 mt-1">
                    {item.dropdown.map((sub, i) => {
                      const isRouterLink = sub.href.startsWith('/') && !sub.href.includes('#');
                      const linkClasses = "block px-3 py-2 text-sm font-medium text-gray-400 hover:text-white transition-colors";
                      
                      if (isRouterLink) {
                        return (
                          <Link key={i} to={sub.href} className={linkClasses} onClick={() => setMobileMenuOpen(false)}>{sub.name}</Link>
                        );
                      }
                      return (
                        <a key={i} href={sub.href} className={linkClasses} onClick={() => setMobileMenuOpen(false)}>{sub.name}</a>
                      );
                    })}
                  </div>
                </div>
