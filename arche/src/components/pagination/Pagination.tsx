...
const Pagination: React.FC<PaginationProps> = ({
  ...
  const baseBtn =
    "w-9 h-9 flex items-center justify-center border text-[10px] tracking-[1px] transition-all duration-300 bg-transparent";
  ...
  return (
    <div className="flex items-center justify-center gap-[2px] py-8">
      ...
      <button
        ...
        className={`${baseBtn} border-white/10 text-white/40 hover:border-white/30 hover:text-[#f0ede6] disabled:opacity-30`}
      >
        «
      </button>
      ...
      {getPages().map((page, i) => (
        <button
          ...
          className={`${baseBtn} font-montserrat ${
            page === currentPage
              ? "border-[#c9a96e] text-[#c9a96e]"
              : page === "..."
                ? "border-transparent text-white/30 cursor-default"
